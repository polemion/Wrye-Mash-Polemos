...
class SettingsTabs(object):
    """Settings Tabs."""

    def __init__(self):
        """Init."""
        self.openmw = conf.settings['openmw']
        self.TabNames = {0: 'General', 1: 'Paths', 2: 'Defaults', 3: 'Advanced',
                         4: 'About'}  # Keep this updated for tab events.
        # Layout
    ...
    def generalTab(self):
        ...
        Update_Sizer.AddMany([(self.Update_po, 1, wx.ALL, 5), (self.Update_staticText, 0, wx.ALL, 5),
        ...
        IntOpt1_Sizer.AddMany(
            [(self.MinOnClose, 0, wx.ALL, 5), ((0, 0), 1, wx.EXPAND, 5), (self.ShowErr, 0, wx.ALL, 5)])
        IntOpt2_Sizer.AddMany([(self.InterfaceStatic, 0, wx.ALIGN_CENTER | wx.ALL, 5), ((30, 0), 0, 0, 5),
                               (self.ThemeChoice, 1, wx.ALL, 5)])
        IntOpt3_Sizer.AddMany(
            [(self.EncodStatic, 0, wx.ALIGN_CENTER | wx.ALL, 5), ((30, 0), 0, 0, 5), (self.EncodChoice, 1, wx.ALL, 5)])
        Interface_Sizer.AddMany([(IntOpt0_Sizer, 0, wx.EXPAND, 5),
                                 (IntOpt1_Sizer, 0, wx.EXPAND, 5), (IntOpt2_Sizer, 0, wx.EXPAND, 5),
                                 (IntOpt3_Sizer, 0, wx.EXPAND, 5)])
        General_Sizer = wx.BoxSizer(wx.VERTICAL)
        General_Sizer.AddMany([(Menu_Sizer, 0, wx.EXPAND | wx.ALL, 5), (Update_Sizer, 0, wx.EXPAND | wx.ALL, 5),
    ...
    def pathsTab(self):
        ...
        if not self.openmw:  # Regular Morrowind support
            ...
            OptionalPaths_Sizer.AddMany(
                [(Mlox_Sizer, 0, wx.EXPAND, 5), (MGEXE_Sizer, 0, wx.EXPAND, 5), (TES3cmd_Sizer, 0, wx.EXPAND, 5)])
            Paths_Sizer.AddMany([(MainPaths_Sizer, 0, wx.EXPAND | wx.ALL, 5), ((0, 0), 1, 0, 5),
        ...
        if self.openmw:  # OpenMW/TES3mp support
            ...
            OptionalPaths_Sizer.AddMany([(DataFiles_Sizer, 0, wx.EXPAND, 5), (TES3mpconf_Sizer, 0, wx.EXPAND, 5),
                                         (Mlox64_Sizer, 0, wx.EXPAND, 5)])
            Paths_Sizer.AddMany(
                [(MainPaths_Sizer, 0, wx.EXPAND | wx.ALL, 5), (OptionalPaths_Sizer, 0, wx.EXPAND | wx.ALL, 5)])
    ...
    def aboutTab(self):
        ...
        AboutImageBtn_Sizer.AddMany([(self.license_button, 0, wx.RIGHT | wx.LEFT, 5), ((11, 0), 0, 0, 5),
        ...
        About_Sizer.AddMany([(self.title, 0, wx.EXPAND | wx.RIGHT | wx.LEFT,
                              5), (self.version, 0, wx.EXPAND | wx.RIGHT | wx.LEFT, 5),
...
class SettingsWindow(wx.Dialog, SettingsTabs):  # Polemos: Total reconstruction.
    ...
    def __init__(self, parent=None, id=wx.ID_ANY, size=(-1, -1), pos=dPos,
                 style=wx.STAY_ON_TOP | wx.DEFAULT_DIALOG_STYLE, settings=None):
        """Settings Dialog."""
        ...
        if True:  # Theming
            # Fonts
            self.title.SetFont(
                wx.Font(12, wx.FONTFAMILY_SWISS, wx.FONTSTYLE_NORMAL, wx.FONTWEIGHT_BOLD, False, '@Arial Unicode MS'))
            self.version.SetFont(
                wx.Font(10, wx.FONTFAMILY_SWISS, wx.FONTSTYLE_NORMAL, wx.FONTWEIGHT_BOLD, False, '@Arial Unicode MS'))
            self.home_url.SetFont(
                wx.Font(9, wx.FONTFAMILY_SWISS, wx.FONTSTYLE_NORMAL, wx.FONTWEIGHT_BOLD, True, '@Arial Unicode MS'))
        ...
        if True:  # Init Conditions
            ...
            if self.openmw:  # OpenMW
                [x.Disable() for x in
                 (self.Mlox64_static, self.fldmlox64, self.btnBrowsemlox64, self.TES3mpConfigs_static,
    ...
    def txtWrap(self):
        ...
        # Common
        [x.Wrap(-1) for x in
         (self.Update_staticText, self.InterfaceStatic, self.title, self.version, self.Settingstext)]
        if not self.openmw:  # Regular Morrowind support
            [x.Wrap(-1) for x in (self.Morrowind_static, self.Installers_static, self.Mlox_static)]
        if self.openmw:  # OpenMW/TES3mp support
            [x.Wrap(-1) for x in
             (self.OpenMWTES3mp_static, self.Downloads_static, self.Mods_static, self.OpenMWConfigs_static,
    ...
    def check_conflicts(self):
        ...
        # OpenMW/Tes3MP
            ...
            check = (self.fldOpenMWloc, self.flddatamods, self.fldDownloads, self.fldDataFiles)
    ...
    def set_settings(self):
        ...
        [x for num, x in enumerate(self.themeData) if num == self.ThemeChoice.GetSelection()][0]
        conf.settings['profile.encoding'] = self.EncodChoiceNew = [enc for enc in profileEncodings if enc in
                                                                   [x for num, x in enumerate(self.EncodChoiceList) if
                                                                    num == self.EncodChoice.GetSelection()][0]][0]
    ...
    def chkSpecialPaths(self):
        ...
        if not self.openmw:  # Polemos: Regular Morrowind Settings:
            ...
            if conf.settings['openmwDir'] != self.fldOpenMWloc.GetValue():
                ...
                return True
            elif conf.settings['openmwprofile'] != os.path.dirname(self.fldOpenMWConf.GetValue()):
                return True
            elif conf.settings['openmw.datafiles'] != self.fldDataFiles.GetValue():
                return True
            elif conf.settings['TES3mpConf'] != self.fldTES3mpConf.GetValue():
                return True
            elif conf.settings['mashdir'] != os.path.join(self.flddatamods.GetValue(), 'Mashdir'):
                return True
    ...
    def InitCommon(self):
        ...
        try:
            self.EncodChoice.SetSelection(
                [num for num, x in enumerate(self.EncodChoiceList) if conf.settings['profile.encoding'] in x][0])
        except:
            self.EncodChoice.SetSelection(
                [num for num, x in enumerate(self.EncodChoiceList) if defaultEncoding in x][0])
    ...
    def InitSettings(self):
        ...
        if self.openmw:  # OpenMW/Tes3MP Settings
            # Paths
            for x, y in zip((self.fldOpenMWloc, self.flddatamods, self.fldDownloads, self.fldOpenMWConf,
                             self.fldDataFiles, self.fldTES3mpConf, self.fldmlox64),
