...
def setCheckListItems(gList, names, values):  # Polemos - Installers: break if not read.
    ...
    if not names:
        ...
        for index in range(gList.GetCount(), len(names), -1): gList.Delete(index - 1)
...
def askOpen(parent, title='', defaultDir='', defaultFile='', wildcard='', style=wx.FD_OPEN):
    ...
    if dialog.ShowModal() != wx.ID_OK:
        ...
        result = [GPath(pth) for pth in dialog.GetPaths()]
    else:
...
def showWryeLog(parent, logText, title='', style=0, asDialog=True, icons=None):
    ...
    # --Dialog or Frame
    if asDialog:
        window = wx.Dialog(parent, defId, title, pos=pos, size=size, style=wx.DEFAULT_DIALOG_STYLE | wx.RESIZE_BORDER)
    else:
        window = wx.Frame(parent, defId, title, pos=pos, size=size,
                          style=(wx.RESIZE_BORDER | wx.CAPTION | wx.SYSTEM_MENU | wx.CLOSE_BOX | wx.CLIP_CHILDREN))
        if icons: window.SetIcons(icons)
...
class Tank(wx.Panel):  # Polemos: Edits
    ...
    def UpdateColumns(self):
        ...
        for index, column in enumerate(columns):
            ...
            if conf.settings['mash.large.fonts']:
                ...
                self.gList.SetFont(wx.Font(*interface.internalStyle['big.font']))
    ...
    def UpdateItem(self, index, item=None, selected=tuple()):
        ...
        if textKey:  # Polemos: Theme engine
            if textKey == 'BLACK' and interface.style['lists.font.color'] is not None:
                gItem.SetTextColour(interface.style['lists.font.color'])
            else:
                gItem.SetTextColour(colors[textKey])
        else:
            if interface.style['lists.font.color'] is None:
