...
from . import compat, mush, appinfo
# Exceptions
...
from .merrors import MaxLoadedError as MaxLoadedError, SortKeyError as SortKeyError, \
...
class Settings(object):  # Polemos: Added revert to backup configuration.
    ...
    def __init__(self, confFile='settings.pkl'):
        ...
        if not os.path.exists(self.confFile) and os.path.exists('%s%s' % (self.confFile, '.bak')):
            shutil.copyfile('%s%s' % (self.confFile, '.bak'), self.confFile)
            print('%s: [Warning] Configuration not found, will use backup.' % datetime.now())
        # --Load
        if os.path.exists(self.confFile):
            inData = compat.uncpickle(self.confFile)
    ...
    def save(self):
        ...
        pickle.dump(outData, open(tempPath, 'wb'), -1)
...
class Table(object):
    ...
    def save(self):  # Polemos: Unicode fix. Strange one. Was it mine? Questions, questions, questions... (a)
        ...
        if self.hasChanged:
            ...
            pickle.dump(self.data, open(tempPath, 'wb'), -1)
            renameFile(tempPath, filePath, True)
            self.hasChanged = False
...
class Cell(Record):
    ...
    def cmpId(self, other):
        ...
        # --Interior cells?
            ...
            return (self.cellName > other.cellName) - (self.cellName < other.cellName)
        # --Exterior cells?
        elif self.gridX != other.gridX:
            return (self.gridX > other.gridX) - (self.gridX < other.gridX)
        else:
            return (self.gridY > other.gridY) - (self.gridY < other.gridY)
...
class Fmap(Record):
    ...
    def edit(self):
        ...
        if not self.mapd:
            data = self.data[24:]
            mapd = self.mapd = []
            for index in range(0, 3 * wmap * wmap, 3):
...
class MWIniFile(object):  # Polemos: OpenMW/TES3mp support
    ...
    def criticalMsg(self, msg, dtype='', modal=True):
        ...
        dialog.ErrorMessage(None, msg, dtype=dtype, modal=modal)

    def getSetting(self, section, key, default=None):
        """Gets a single setting from the file."""
        section, key = (bolt.LString(item) for item in (section, key))
    ...
    def saveSettings(self, settings):
        ...
        settings = {LString(x): {LString(u): v for u, v in y.items()} for x, y in settings.items()}
        reComment = re.compile(';.*')
        reSection = re.compile(r'^\[\s*(.+?)\s*\]$')
        reSetting = re.compile(r'(.+?)\s*=')
    ...
    def openmw_plugin_factory(self):  # Polemos: OpenMW/TES3mp
        ...
        esm_order_active = [x for x in plugins_order_paths if
                            x.lower().endswith('.esm') or x.lower().endswith('.omwgame')]
        esp_order_active = [x for x in plugins_order_paths if
            ...
            if not self.openmw:  # Morrowind
                ...
                for line in self.confLoadLines:
                    if line == self.ArchiveSection:
                        ...
                        for aNum in range(len(self.bsaFiles)):
                        ...
                        for pNum in range(len(self.loadFiles)):
    ...
    def safeSave(self, simpleData=False):  # Polemos
        ...
        try:
            ...
            from .gui import dialog
            dialog.ErrorMessage(None, err.message)
            self.restoreBackup()
    ...
    def refreshDoubleTime(self):
        ...
        for loadFile in self.loadFiles:
            try:  # Polemos: Black magic here, move along.
                ...
                doubleTime[mtime] = mtime in doubleTime
            except:
                pass
    ...
    def sortLoadFiles(self):
        ...
        self.loadFiles.sort()
        self.loadFiles.sort(key=lambda x: x[-3:].casefold())
...
class FileInfo(object):  # Polemos: OpenMW/TES3mp support
    """Abstract TES3 File."""
    ...
    def getHeader(self):
        ...
        try:
            ...
            ins.close()
            raise Tes3Error(self.name, 'Struct.error: ' + repr(rex))
        except Tes3Error as error:
            ins.close()
...
class FileInfos(object):  # + OpenMW/TES3mp support
    ...
    def refresh_Morrowind(self):
        ...
        deleted = []
        if not os.path.exists(self.dir): os.makedirs(self.dir)
        for fileName in os.listdir(self.dir):  # --Loop over files in directory
            fileName = fileName
            # --Right file type?
            filePath = os.path.join(self.dir, fileName)
            if not os.path.isfile(filePath) or not self.rightFileType(fileName): continue
...
class ModInfos(FileInfos):
    ...
    def saveObjectMaps(self):
        ...
        pickle.dump(self.objectMaps, open(path, 'wb'), -1)


# ------------------------------------------------------------------------------

class ReplJournalDate(object):
...
class UtilsData(DataDict):  # Polemos: Many changes here.
    ...
    def save(self):  # Polemos: fixes
        ...
        if changed:  # Items removed
            [lines.remove(line) for key in orgData.keys() if key not in self.data.keys() for line in lines if
             line.startswith(key)]
    ...
    def refresh(self):
        ...
        for fileName in self.dir.list():
            ...
            if maImageExt and filePath.isfile():
                newData[fileName] = (
                maImageExt.group(1).lower(), filePath.mtime, os.path.getsize(filePath._s))  # Polemos: Added size attr.
...
class datamod_order(object):  # Polemos: OpenMW/TES3mp support
    ...
    def datamods(self):
        ...
        moddir = settings['datamods']
        datamods = [join(moddir, dir) for dir in os.listdir(moddir)]
        # DataMods final list
...
class BSA_order(object):  # Polemos
    """Create BSA entries."""
    ...
    def bsa_files(self):
        ...
        if not self.openmw:
            bsas = [os.path.join(self.dir, bsafile) for bsafile in os.listdir(self.dir) if
                    bsafile.endswith('.bsa')]
        if self.openmw:
            bsas = []
            bsadir = self.bsa_dir()[:]
            ...
            [[bsas.append(os.path.join(moddir, bsafile)) for bsafile in os.listdir(moddir)
...
class Packages_Factory(object):  # Polemos
    ...
    def package_files(self):
        ...
        packages = [os.path.join(self.dir, package) for package in os.listdir(self.dir) if
                    package.endswith(('.zip', 'rar', '7z'))]
        return packages
...
class GetBckList(object):  # Polemos
...
class LoadModOrder(object):  # Polemos
    ...
    def dataFactory(self, rawData):
        ...
        if self.mode == 'modsnap':
            ...
            self.modData = [(int(x[0]), GPath(x[1].replace('"', '')), int(x[2])) for x in
                            [y.split(':') for y in rawData]]
...
class SaveModOrder(object):  # Polemos
...
class CopyTree(object):  # Polemos
    ...
    def copyAct(self):
        ...
        import wx
        from .gui import dialog
        self.dialog = dialog.GaugeDialog(self.parent, self.target_dir, self.title, self.filesLen)
        self.dialog.Show()
    ...
    def treeOp(self):  # Polemos: This is a very fast implementation. Todo: post it on stackoverflow?
        ...
        try:
            cnt = 0
            for root, dirsn, files in os.walk(source_dir, topdown=False):
...
class ResetTempDir(object):  # Polemos
    ...
    def safe(self, tempdir):
        ...
        if [x for x in os.listdir(tempdir) if x in ['7z.exe', 'mash.exe', 'mash.py']]: return False
        return True
...
class Installer(
    object):  # Polemos: added MWSE compatibility, optimised, bug fixing, restored lost Bain func on many packages.
    ...
    @staticmethod
    def refreshSizeCrcDate(apRoot, old_sizeCrcDate, progress=None, removeEmpties=False,
                           fullRefresh=False):  # Polemos: fixed crc bug, +speed, more.
        ...
        ghost_norm = {y: x for x, y in norm_ghost.items()}
    ...
    def refreshDataSizeCrc(self):
        ...
        for full, size, crc in self.fileSizeCrcs:
            ...
            # --Skip file?
            if (rootLower == 'omod conversion data' or fileLower[-9:] == 'thumbs.db' or fileLower[
                                                                                        -11:] == 'desktop.ini'):
        ...
        if self.isActive and data_sizeCrc != old_sizeCrc:
            ...
            for file, sizeCrc in old_sizeCrc.items():
    ...
    def refreshStatus(self, installers):
        ...
        if self.type == 0:
            ...
            for file, sizeCrc in data_sizeCrc.items():
...
class InstallerArchive(Installer):
    ...
    def unpackToTemp(self, archive, fileNames, progress=None):  # Polemos fixes and addons.
        ...
        command = r'7z.exe x "%s" -bb -y -o"%s" @%s -scsUTF-8' % (apath.s, self.tempDir.s, self.tempList.s)
        args = shlex.split(command)
...
class InstallerProject(Installer):
    ...
    def removeEmpties(self, name):
        ...
        for asDir, sDirs, sFiles in os.walk(
                projectDir.s):  # Polemos: replaced os.walk which is slow in Python 2.7 and below.
    ...
    def syncToData(self, package, projFiles):
        ...
        srcProj = tuple((x, y) for x, y in self.refreshDataSizeCrc().items() if x in projFiles)
...
class InstallersData(bolt.TankData, DataDict):  # Polemos fixes
    ...
    def refresh(self, progress=None, what='DIONS',
                fullRefresh=False):  # D.C.-G. Modified to avoid system error if installers path is not reachable.
        """Refresh info."""
            ...
            if column == 'Package':
                ...
                if column in ('Package', 'Group'):
                    ...
                    try:
                        ...
                        value = '%sKB' % formatInteger(value // 1024)
                else:
    ...
    def refreshRenamed(self):
        ...
        if not pRenamed.exists():
            ...
            for fields in reader:
                ...
                off, local = (field.strip() for field in fields[:2])
                if not reModExt.search(off) or not reModExt.search(local): continue
                off, local = (GPath(item) for item in (off, local))
        ...
        if changed:
            for installer in self.data.values():
        ...
        if not self.openmw:  # Polemos: Regular Morrowind support
            ...
            for subPending, iClass in zip((pending - projects, pending & projects),
                ...
                for index, package in enumerate(sorted(subPending)):
                    ...
                    try:
                        installer.refreshBasic(apath, SubProgress(progress, index, index + 1))
    ...
    def refreshOrder(self):
        ...
        data = self.data
        ordered, pending = [], []
        for archive, installer in self.data.items():
    ...
    def install(self, archives, progress=None, last=False, override=True):
        ...
        for index, archive in enumerate(archives):
            ...
            if destFiles: installer.install(archive, destFiles, self.data_sizeCrcDate,
    ...
    def uninstall(self, unArchives, progress=None):
        ...
        if ['mash.installers.autoAnneal'] and restoreArchives:
            ...
            for index, archive in enumerate(restoreArchives):
                progress(index, archive.s)
                installer = data[archive]
                destFiles = {x for x, y in restores.items() if y == archive}
                if destFiles: installer.install(archive, destFiles, data_sizeCrcDate,
                                                SubProgress(progress, index, index + 1))
        # --Done
        ...
        * Install missing files from active anPackages."""
        ...
        for package in sorted(data, key=getArchiveOrder, reverse=True):
            ...
            if installer.isActive:
                ...
                myRestores = (removes & files) - set(restores)
                for file in myRestores:
                    if installer.data_sizeCrc[file] != data_sizeCrcDate.get(file, (0, 0, 0))[:2]: restores[
                        file] = package
                    removes.discard(file)
        # --Remove files
        emptyDirs = set()
        ...
        if restoreArchives:
            ...
            for index, package in enumerate(restoreArchives):
                ...
                installer = data[package]
                destFiles = {x for x, y in restores.items() if y == package}
                if destFiles: installer.install(package, destFiles, data_sizeCrcDate,
                                                SubProgress(progress, index, index + 1))
    ...
    def getConflictReport(self, srcInstaller, mode):
        ...
        for package in sorted(self.data, key=getArchiveOrder):
            ...
            curConflicts = Installer.sortFiles([x.s for x, y in installer.data_sizeCrc.items()
...
class FileRep(object):
    ...
    def load(self, keepTypes='ALL', factory={}):
        ...
        ins = Tes3Reader(self.fileInfo.name, open(inPath, 'rb'))
        (name, size, delFlag, recFlag) = ins.unpackRecHeader()
        self.tes3 = Tes3(name, size, delFlag, recFlag, ins, True)
        # --Raw data read
        while not ins.atEnd():
    ...
    def sortRecords(self):
        ...
        # --Sort
        self.records.sort(key=lambda x: (typeOrder[x.name], ids[x]))
...
class FileRefs(FileRep):
    ...
    def removeOrphanContents(self):
        ...
        self.conts_id = {id: record for id, record in self.conts_id.items() if matched[id] > 0}
        return len(orphans)
...
class FileDials(FileRep):
    ...
    def dumpText(self, textFileName, groupBy='spId', spId=None):
        ...
        # --Text File
        with open(textFileName, 'wb') as textFile:
...
class FileScripts(FileRep):
    ...
    def loadText(self, textFileName):
        """Replaces dialog text with text read from file."""
        with open(textFileName, 'rb') as textFile:
            ...
            for line in textFile:
                ...
                if maHeader:
                    ...
                    lines.append(line)
        unBuffer()
        return sorted(changed, key=str.lower)
...
class CharSetImporter(object):
    ...
    def loadText(self, fileName):
        ...
        statNames = (
        'Agility', 'Block', 'Light Armor', 'Marksman', 'Sneak', 'Endurance', 'Heavy Armor', 'Medium Armor', 'Spear',
...
class ScheduleGenerator(object):
            ...
            for line in ins:
                ...
                # --Data
                    ...
                    # --NPC
                        ...
                        coords[-1] = repr(int(coords[-1]) * 57)  # --Workaround interior rotation bug
    ...
    def getCycleScript(self, town, cycle):
        ...
        out.write(self.tsCycle1.substitute(town=town, cycle=repr(cycle), cycleCode=cycleCode))
        for npc in npcs:
